...
4. `scripts/setup.sh` 실행 중 chat id 자동 탐색을 선택하면 최근 메시지에서 chat id를 찾아 `.env`에 저장합니다.
...
설치 과정에서 Telegram bot token, timezone을 입력합니다. chat id는 자동으로 찾을 수 있고, 자동 탐색이 실패하면 직접 입력하면 됩니다. 마지막에 macOS 자동 실행을 설치할지 선택할 수 있습니다.
